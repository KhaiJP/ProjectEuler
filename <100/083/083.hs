...
main = do
    ...
    let answer = resultMap M.! (size', size')
    print answer
    hClose handle
...
adjointPairs = l0 ++ l1
    where
        l1  = filter (\(_, (r, c)) -> 1<=r && r<=size' && 1<=c && c<=size') . concat $ l1'
        l1' = [[((r, c), (r+1, c)), ((r, c), (r-1, c)), ((r, c), (r, c+1)), ((r, c), (r, c-1))] | r <- [1..size'], c <- [1..size']]
        l0  = [((0, 0), (1, 1))]
